...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * Created by Murphy.lee on 17/4/21.
	 */
	var setting = __webpack_require__(1456);
	new Vue({
	  el: "#container",
	  data: function () {
	    return {
...
/***/ function(module, exports, __webpack_require__) {
	...
	// </template>
	// <script>
	var a_setting_item = __webpack_require__(1458);
	exports.default = {
	  data: function data() {
	    return {
	      data: [{
	      ...
	      bind_data: [{
	        ...
	        name: "微信账号",
	        url: "http://open." + window.location.host.split(".")[1] + "." + window.location.host.split(".")[2] + "/WechatLogin/index?bind=1&referer=" + location.href,
	        tj_id: "bind_wechart",
	        bind: ""
	      }, {
	        name: "解绑银行卡",
	  ...
	  methods: {
	    init: function init() {
	      ...
	      $.ajax({
	        type: 'POST',
	        url: sccope.dataUrl,
	        data: _layout2.default.strSign("", {}),
	        dataType: "json",
	        success: function success(result) {
	          if (result.code) {} else {
	            sccope.bind_data[0].bind = result.data.mobile;
	            if (result.data.mobile == '') {} else {
...
/***/ function(module, exports) {
	...
	// <template>
	//   <div>
	//     <a v-for="item in datalist" class="user_menu_item" :href="item.url"
	//        :data-dav-tj="'setting|'+item.tj_id+'|'+item.tj_id+'|1|'+item.tj_id+'@setting'">
	//       <span>{{item.name}}</span>
	//       <span class="child pull-right">
	//         <i class="icon dav_icon_dayuhao_18_38"></i>
	//       </span>
	//       <span v-if="item.hasOwnProperty('bind')" class="f12 pull-right">{{item.bind == "" ? '未绑定' : item.bind}}</span>
	//     </a>
	//   </div>
	// </template>
	// <script>
	exports.default = {
	  ...
	  //   }
	  //
	  //   .user_menu_item .child {
	  //     overflow: hidden;
	  //   }
	  //
	  //   .user_menu_item .child i.icon {
	  //     display: inline-block;
	  //     vertical-align: 0;
	  //     width: 8px;
	  //     height: 8px;
