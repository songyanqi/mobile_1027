...
/***/ function(module, exports) {
	...
	$(function () {
	  ...
	  window.bravetime.callNativeReady = function () {
	    ...
	    bravetime.callNative(function () {}, function () {}, "base", "ready", []);
	  };

	  window.bravetime.callNativeHoldPic = function (src) {
	    bravetime.callNative(function () {}, function () {}, "base", "savePic", [src]);
	  };
	  ...
	  window.bravetime.info = function (msg) {
	    ...
	    if (infoContainer.length) {
	      ...
	      infoContainer = $('<div style="width:100%;z-index: 99;max-width:640px;position: fixed;top: 200px;text-align: center;"><div class="inner" style="display:inline-block;padding:12px 14px;max-width:80%;background: #D73C6B;color:#fff;border-radius: 3px;">' + msg + '</div></div>').addClass("bravetime-info");
	  ...
	  function add2cart(goods_id, number, callback, _error, me) {
	    ...
	    $.ajax({
	      ...
	      success: function success(res) {
	        ...
	        for (var i = 0; i < data.length; i++) {
	          ...
	          str += '<div class="goods_item">' + '<a href="/' + d.goods_id + '.html?rl=' + res.referer.rl + '&rp=' + res.referer.rp + '">' + '<div class="goods_img">' + (i < 4 ? '<img src=\"' + d.goods_img + '\">' : '<img src="http://pic.davdian.com/free/loading_320_320_v2.png" data-original=\"' + d.goods_img + '\">') + (d.goods_label && d.goods_label.length ? '<span class="img_label">' + d.goods_label + '</span>' : "") + '</div>' + '<div class="goods_name">' + d.goods_name + '</div>' + '<div class="goods_price">' + '<span class="price">¥' + d.shop_price1 + '<small>' + d.shop_price2 + '</small>' + '</span>' + (d.seller_income != '0' ? '<span class="vip_return">' + '<span class="vip_return_title">会员返</span>' + '<span class="vip_return_f">¥</span> ' + '<span class="vip_return_price">' + d.seller_income + '</span>' + '</span>' : "") + '</div>' + '</a>' + '</div>';
	        }
	        str += '</div>';
	        $el.html(str);
	...
	if ($("a.tag").length) {
	  var active = function active(name) {
